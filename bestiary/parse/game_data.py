...
class _DataTables:
    filename = 'bestiary/com2us_data/localvalue.dat'
    _tables = {}
    _num_tables = None
    _data_start = None
    _table_offsets = {}
    ...
    def __getitem__(self, key):
        if key not in _DataTables._tables:
            _DataTables._tables[key] = _DataTables._get_table(key)

        return _DataTables._tables[key]

    def __len__(self):
        return _DataTables._get_num_tables()

    @staticmethod
    def _get_num_tables():
        if _DataTables._num_tables is None:
            file = _DataTables._get_decrypted_data()
            file.pos = _DataTables.TABLE_COUNT_POS
            _DataTables._num_tables = file.read(f'intle:32') - 1

        return _DataTables._num_tables

    @staticmethod
    def _get_table(key):
        if key not in _DataTables._tables:
            f = _DataTables._get_decrypted_data()
            start, end = _DataTables._get_table_offsets(key)

            # Set bitstream to start of table data
            f.pos = _DataTables.TABLE_START_POS + start * 8
            ...
            # Store table data
            _DataTables._tables[key] = {}
            for row_string in entire_table[1:]:
                row = row_string.split('\t')
                row_key = json.loads(row[0])
                _DataTables._tables[key][row_key] = {
                    column_headers[col]: value for col, value in enumerate(row)
                }
    ...
    def _get_table_offsets(key):
        if not _DataTables._table_offsets:
            # Store all table offsets
            f = _DataTables._get_decrypted_data()
            f.pos = _DataTables.TABLE_DEFS_POS

            for x in range(_DataTables._get_num_tables()):
                table_num, start, end = f.readlist(['intle:32'] * 3)
                _DataTables._table_offsets[table_num] = (start, end)

            _DataTables.TABLE_START_POS = f.pos

        return _DataTables._table_offsets[key]

    @staticmethod
    def _get_decrypted_data():
        if _DataTables._decrypted_data is None:
            with open(_DataTables.filename) as f:
                _DataTables._decrypted_data = decrypt_response(f.read().strip('\0'))

        return ConstBitStream(_DataTables._decrypted_data)
